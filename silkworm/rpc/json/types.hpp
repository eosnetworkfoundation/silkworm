        ...
        if (json.is_object()) {
            if (json.contains("blockHash")) {
                ...
                if (hashString.is_string()) {
                    ...
                    if (s.length() == 66) {
                        return silkworm::rpc::BlockNumOrHash{s};
                    }
                }
                throw std::system_error{std::make_error_code(std::errc::invalid_argument), "BlockNumOrHash: blockHash"};
            } else if (json.contains("blockNumber")) {
                const auto& num = json["blockNumber"];
                if (num.is_number()) {
                    return silkworm::rpc::BlockNumOrHash{num.get<std::uint64_t>()};
                }
                ...
                throw std::system_error{std::make_error_code(std::errc::invalid_argument), "BlockNumOrHash: blockNumber"};
            }
        }
        throw std::system_error{std::make_error_code(std::errc::invalid_argument), "BlockNumOrHash: unsupported input type"};
    }
};
