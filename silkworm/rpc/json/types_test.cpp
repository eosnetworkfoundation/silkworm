...
TEST_CASE("deserialize block_num_or_hash", "[silkworm::json][from_json]") {
    ...
    SECTION("as objct with blockHash") {
        ...
        auto bnoh = json.get<BlockNumOrHash>();
    ...
    SECTION("as invaild type") {
        ...
        auto bnoh = json.get<BlockNumOrHash>();
            ...
            CHECK(std::string(e.what()) == "BlockNumOrHash: unsupported input type: Invalid argument");
            exception = true;
        }
        CHECK(exception);
    }
    ...
    SECTION("as object with invliad blockHash") {
        ...
        for (const auto& json : inputs) {
            ...
            auto bnoh = json.get<BlockNumOrHash>();
                ...
                CHECK(std::string(e.what()) == "BlockNumOrHash: blockHash: Invalid argument");
                exception = true;
            }
            CHECK(exception);
        }
    ...
    SECTION("as object with invliad blockNumber") {
        ...
        for (const auto& json : inputs) {
            ...
            auto bnoh = json.get<BlockNumOrHash>();
                ...
                CHECK(std::string(e.what()) == "BlockNumOrHash: blockNumber: Invalid argument");
                exception = true;
            }
            CHECK(exception);
        }
