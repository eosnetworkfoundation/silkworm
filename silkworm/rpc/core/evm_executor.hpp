...
class EVMExecutor {
  public:
    ...
    EVMExecutor(const silkworm::Block& block, const silkworm::ChainConfig& config, WorkerPool& workers, std::shared_ptr<State> state, const evmone::gas_parameters& gas_params, const evmone::eosevm::gas_prices& gas_prices)
        : config_(config),
          workers_{workers},
          state_{std::move(state)},
          rule_set_{protocol::rule_set_factory(config)},
          execution_processor_{block, *rule_set_, *state_, config, false, gas_params, gas_prices},
