...
CallManyResult CallExecutor::executes_all_bundles(const silkworm::ChainConfig& config,
                                                  ...
                                                  const evmone::eosevm::gas_prices& gas_prices) {
    CallManyResult result;
    const auto& block = block_with_hash->block;
    auto state = execution::StateFactory{transaction_}.create_state(this_executor, storage, txn_id);

    EVMExecutor executor{block, config, workers_, std::make_shared<state::OverrideState>(*state, accounts_overrides), gas_params, gas_prices};
...
Task<CallManyResult> CallExecutor::execute(
    ...
    const auto [evm_version, gas_params, gas_prices] = co_await load_gas_parameters(transaction_, chain_config, block_with_hash->block);
