...
Task<std::optional<Receipts>> generate_receipts(db::kv::api::Transaction& tx,
    ...
    const auto [evm_version, gas_params, gas_prices] = co_await load_gas_parameters(tx, chain_config, block);
    const auto receipts = co_await async_task(workers.executor(), [&]() -> Receipts {
        ...
        EVMExecutor executor{block, chain_config, workers, state, gas_params, gas_prices};
