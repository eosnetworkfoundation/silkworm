...
TEST_CASE("Storage update") {
    ...
    const Bytes key{storage_prefix(address.bytes, kDefaultIncarnation)};
    ...
    upsert_storage_value(*state, key, ByteView{location_a.bytes}, value_a1.bytes);
    upsert_storage_value(*state, key, ByteView{location_b.bytes}, value_b.bytes);
