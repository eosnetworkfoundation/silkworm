...
Stage::Result HashState::hash_from_account_changeset(RWTxn& txn, BlockNum previous_progress, BlockNum to) {
    ...
    try {
        ...
        auto changeset_data = source_changeset->find(to_slice(source_initial_key), /*throw_notfound=*/false);
...
Stage::Result HashState::unwind_from_account_changeset(RWTxn& txn, BlockNum previous_progress, BlockNum to) {
    ...
    try {
        ...
        // A block can contain no accounts changes (no mining reward) in it so we don't throw an exception in that case
        auto changeset_data = changeset_cursor->find(to_slice(initial_key), /*throw_notfound=*/false);
...
void HashState::write_changes_from_changed_addresses(RWTxn& txn, const ChangedAddresses& changed_addresses) {
    throw_if_stopping();
    if(changed_addresses.size() == 0) return;
