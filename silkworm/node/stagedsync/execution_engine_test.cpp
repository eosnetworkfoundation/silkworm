...
TEST_CASE("ExecutionEngine") {
    ...
    SECTION("fork A->B->A") {
        ...
        auto verification = runner.run(exec_engine.verify_chain(block3->header.hash()));
        REQUIRE(holds_alternative<ValidChain>(verification));
        ...
        verification = runner.run(exec_engine.verify_chain(block2b->header.hash()));
        REQUIRE(holds_alternative<ValidChain>(verification));

        // insert block3 again and verify
        exec_engine.insert_block(block3);
        verification = runner.run(exec_engine.verify_chain(block3->header.hash()));
        REQUIRE(holds_alternative<ValidChain>(verification));
