...
ValidationResult EthashRuleSet::validate_extra_data(const BlockHeader& header) const {
    ...
    if (chain_config_->_dao_block && chain_config_->dao_block() <= header.number &&
...
BlockReward EthashRuleSet::compute_reward(const Block& block) {
    ...
    const evmc_revision rev{chain_config_->revision(block.header)};
...
intx::uint256 EthashRuleSet::difficulty(
    ...
    const evmc_revision rev = config.revision(BlockHeader{.number=block_num, .timestamp=block_timestamp});
    ...
    if (config._gray_glacier_block.has_value() && block_num >= config.gray_glacier_block()) {
        // EIP-5133: Delaying Difficulty Bomb to mid-September 2022
        bomb_delay = 11'400'000;
    } else if (config._arrow_glacier_block.has_value() && block_num >= config.arrow_glacier_block()) {
    ...
    } else if (config._muir_glacier_block.has_value() && block_num >= config.muir_glacier_block()) {
