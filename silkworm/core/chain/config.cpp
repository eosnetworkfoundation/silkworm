...
nlohmann::json ChainConfig::to_json() const noexcept {
    ...
    std::visit(
        Overloaded{
            [&](const protocol::TrustConfig&) { ret.emplace("trust", nlohmann::json{}); },
    ...
    if (_terminal_total_difficulty.has_value()) {
    ...
    member_to_json(ret, "pragueTime", _prague_time);
...
bool ChainConfig::valid_pre_merge_config() const noexcept {
    ...
    const bool has_merge_at_genesis{!_terminal_total_difficulty || *_terminal_total_difficulty == 0};
...
std::optional<ChainConfig> ChainConfig::from_json(const nlohmann::json& json) noexcept {
    ...
    if (json.contains("ethash")) {
        ...
        config.rule_set_config = protocol::TrustConfig{};
    } else {
    ...
    read_json_config_member(json, "pragueTime", config._prague_time);
...
bool ChainConfig::is_london(BlockNum block_num) const noexcept {
    return (_london_block && block_num >= _london_block);
}

evmc_revision ChainConfig::determine_revision_by_block(uint64_t block_num, uint64_t block_time) const noexcept {
    if (_prague_time && block_time >= _prague_time) return EVMC_PRAGUE;
    ...
    if (_london_block && block_num >= _london_block) return EVMC_LONDON;
    if (_berlin_block && block_num >= _berlin_block) return EVMC_BERLIN;
    if (_istanbul_block && block_num >= _istanbul_block) return EVMC_ISTANBUL;
    if (_petersburg_block && block_num >= _petersburg_block) return EVMC_PETERSBURG;
    if (_constantinople_block && block_num >= _constantinople_block) return EVMC_CONSTANTINOPLE;
    if (_byzantium_block && block_num >= _byzantium_block) return EVMC_BYZANTIUM;
    if (_spurious_dragon_block && block_num >= _spurious_dragon_block) return EVMC_SPURIOUS_DRAGON;
    if (_tangerine_whistle_block && block_num >= _tangerine_whistle_block) return EVMC_TANGERINE_WHISTLE;
    if (_homestead_block && block_num >= _homestead_block) return EVMC_HOMESTEAD;

    return EVMC_FRONTIER;
}

evmc_revision ChainConfig::revision(const BlockHeader& header) const noexcept {
    if(!std::holds_alternative<protocol::TrustConfig>(rule_set_config)) {
...
std::vector<BlockTime> ChainConfig::distinct_fork_times() const {
    ...
    ret.insert(_prague_time.value_or(0));
...
constinit const ChainConfig kHoleskyConfig{
    ...
    ._shanghai_time = 1696000704,
    ._cancun_time = 1707305664,
...
constinit const ChainConfig kBorMainnetConfig{
    ...
    ._constantinople_block = 0,
    ._petersburg_block = 0,
    ._istanbul_block = 3'395'000,
    ._muir_glacier_block = 3'395'000,
    ._berlin_block = 14'750'000,
    ._london_block = 23'850'000,
    .burnt_contract = {
