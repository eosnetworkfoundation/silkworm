...
void do_dump_state(EnvConfig& config, const std::string& evm_state_file_name) {
    if (!config.exclusive) {
        throw std::runtime_error("Function requires exclusive access to database");
    }
    auto env{open_env(config)};
    ...
    auto accounts{open_cursor(txn, table::kPlainState)};
    ...
    while (data) {
        ...
        if(key.size() == 20) {
            ...
            const auto acc_res = state::AccountCodec::from_encoded_storage(value);
            success_or_throw(acc_res);
            Account account{*acc_res};
            ...
            if(current_account.has_value() && address == current_account->address) {
                ...
                //dump somewhere else?
                std::cout << to_hex(from_slice(data.key)) << ":" << to_hex(from_slice(data.value)) << std::endl;
            }
        } else {
...
int main(int argc, char* argv[]) {
    ...
    // Dump state
    auto cmd_dump_state = app_main.add_subcommand("dump-state", "Dump EVM state (eos-evm)");
