...
Stage::Result Senders::parallel_recover(db::RWTxn& txn) {
    ...
    try {
        ...
        while (collected_senders_ < total_collected_senders) {
            ...
            // ~100us per secpk1 recover per core
            std::this_thread::sleep_for(std::chrono::microseconds(std::min<uint64_t>((total_collected_senders - collected_senders_) * 20, 10000)));
        }
